...
void argo_reset_coherence(int n){
	...
	for(unsigned long i=0; i<classificationSize; i+=win_granularity){
...
void storepageDIFF(unsigned long index, unsigned long addr){
	...
	if(!have_lock(win_index, homenode)){
		//printf("[%d] Locking mpi_lock_data[%zu][%d].\n", getID(), win_index, homenode);
...
void unlock_windows() {
	// Unlock all windows
	//printf("[%d] Called unlock_windows.\n", getID());
	for(auto p : locked_windows){
		//printf("[%d] Unlocking mpi_lock_data[%d][%d].\n", getID(), p.first, p.second);
